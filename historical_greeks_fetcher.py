...
class ThetaDataRequester(threading.Thread):
    def __init__(self, expiration_date:str):
        ...
        self.ivl = 60000
        self.page_num = 0
        self.url = "http://127.0.0.1:25510/v2/bulk_hist/option/greeks"
    ...
    def fetch_data(self):
        ...
        while page_is_valid:
            ...
            if response.status_code == 200:
                ...
                page_is_valid = False
                succsessful_fetch = False
                break
        return (succsessful_fetch, response.status_code)
...
def main():
    ...
    for year in range(2023, 2025):
        for month in range(1, 13):
            for day in range(1, 32):
                try:
                    if datetime(year, month, day) > datetime(2024, 5, 10):
                        break
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        succsessful_fetches, response_status_code = executor.map(ThetaDataRequester, expiration_dates)
        for fetch_succsess_status in succsessful_fetches:
            total_fetch_count += 1
            if fetch_succsess_status:
                succsessful_fetch_count += 1
            else:
                print("Failed fetch for expiration date " + expiration_dates[total_fetch_count - 1] + " with status code " + response_status_code)
